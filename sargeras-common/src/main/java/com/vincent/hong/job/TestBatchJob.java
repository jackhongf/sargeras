    ...
    protected int dealData(List<User> partListData, AtomicInteger successNum, AtomicInteger pendingNum, AtomicInteger failNum) {
        ...
        for (int i =0; i<partListData.size();i++){
             ...
             if (age%10 ==0){
                 ...
                 System.out.println(threadName+" time: "+ String.valueOf(System.nanoTime())+" ["+threadId+"] pendingNum"+ pendingNum.intValue()+" "+user.toString());
             }else if (age%3 ==0){
                 failNum.getAndIncrement();
                 System.out.println(threadName+" time: "+ String.valueOf(System.nanoTime())+" ["+threadId+"] failNum"+ failNum.intValue()+" "+user.toString());
             }else{
                 successNum.getAndIncrement();
                 System.out.println(threadName+" time: "+ String.valueOf(System.nanoTime())+" ["+threadId+"] successNum"+ successNum.intValue()+" "+user.toString());
